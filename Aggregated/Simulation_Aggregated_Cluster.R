...
for (i in 1:n_simulations) {
  ...
  # Randomly generate animal positions within each cluster
  animal_positions <- data.frame(E = numeric(), N = numeric())
  for (j in 1:n_clusters) {
    animals_in_cluster <- data.frame(
      E = rnorm(total_animals / n_clusters, mean = cluster_centers$E[j], sd = cluster_radius / 2),
      N = rnorm(total_animals / n_clusters, mean = cluster_centers$N[j], sd = cluster_radius / 2)
    )
    animal_positions <- rbind(animal_positions, animals_in_cluster)
  }
  
  # Filter out animals that exceed the boundary
  animal_positions <- animal_positions %>%
    filter(E >= 0 & E <= area_size & N >= 0 & N <= area_size)
...
write.table(ClusteredDesign, "simulation_Aggregated_Clustered_Results.txt", row.names=FALSE, sep="\t")
